...
// ─── Derive root status from sub-items ───
const deriveRootStatus = (item) => {
  ...
  return allSame ? statuses[0] : "inprogress";
};

// ─── Progress (uses sub-item statuses directly, ignores "inprogress") ───
const getOverallProgress = (items) => {
  ...
  // Filter out nulls (inprogress doesn't contribute)
...
const getItemBudget = (item) => {
  if (item.subItems && item.subItems.length > 0) {
    // Root budget is the cap; for donut chart, show the cap
...
function DonutChart({ items, dark }) {
  ...
  const size = 200, cx = size / 2, cy = size / 2, r = 72, stroke = 24;
    ...
    <div style={{ marginTop: 28, display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
      ...
      <div style={{ position: "relative", width: size, height: size }}>
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
          <circle cx={cx} cy={cy} r={r} fill="none" stroke={dark ? "#333" : "#e9ecef"} strokeWidth={stroke} />
          {segments.map((seg) => (
...
function ProgressBar({ progress, dark }) {
  ...
  return (
    <div style={{ marginTop: 20, marginBottom: 8 }}>
      ...
      <div style={{ width: "100%", height: 20, borderRadius: 10, background: dark ? "#2a2a3e" : "#e9ecef", overflow: "hidden", position: "relative", border: `1px solid ${dark ? "#333" : "#ddd"}` }}>
...
// ─── Generic Popup ───
function Popup({ title, children, onClose, dark }) {
  return (
    <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
      <div style={{ background: dark ? "#2a2a3e" : "#fff", borderRadius: 12, padding: "24px 28px", boxShadow: "0 8px 30px rgba(0,0,0,0.2)", maxWidth: 400, width: "90%", color: dark ? "#e0e0e0" : "#1a1a2e" }}>
  ...
  return (
    <Popup dark={dark}>
      ...
      <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
        <button onClick={onCancel} style={{ padding: "8px 18px", fontSize: 13, fontWeight: 600, borderRadius: 6, border: "none", background: dark ? "#444" : "#eee", color: dark ? "#ccc" : "#555", cursor: "pointer" }}>Cancel</button>
...
// ─── Budget Cap Popup (for setting/changing root budget) ───
function BudgetCapPopup({ itemName, currentBudget, onSave, onCancel, dark, showDontAsk, onDontAsk }) {
  const [val, setVal] = useState(currentBudget ? String(currentBudget) : "");
  const [dontAsk, setDontAsk] = useState(false);
  const handleSave = () => {
    const n = parseFloat(val);
    onSave(isNaN(n) || n < 0 ? 0 : Math.round(n * 100) / 100);
  ...
  return (
    <Popup title={`Set budget cap for "${itemName}"`} dark={dark}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 16 }}>
        <span style={{ fontSize: 16, fontWeight: 600, color: dark ? "#8ecae6" : "#2a6f97" }}>$</span>
        <input value={val} onChange={(e) => setVal(e.target.value)} autoFocus placeholder="Enter budget"
          onKeyDown={(e) => e.key === "Enter" && handleSave()}
          style={{ ...getInputStyle(dark), flex: 1, fontSize: 16, padding: "10px 12px" }} />
      </div>
      {showDontAsk && (
...
// ─── Status Badge (for display, not interactive) ───
function StatusBadge({ status }) {
  const info = getStatusInfo(status);
  return (
    <span style={{ fontSize: 11, fontWeight: 600, padding: "3px 6px", borderRadius: 4, background: info.bg, color: info.color, flexShrink: 0 }}>
      {info.label}
    </span>
  );
}
...
function StatusSelect({ value, onChange, small }) {
  ...
  return (
    <select value={value} onChange={(e) => onChange(e.target.value)}
      style={{ fontSize: small ? 10 : 11, fontWeight: 600, padding: small ? "2px 4px" : "3px 6px", borderRadius: 4, border: "none", cursor: "pointer", appearance: "auto", background: info.bg, color: info.color, flexShrink: 0 }}>
  ...
  if (editing) {
    return (
      <div style={{ display: "inline-flex", alignItems: "center", gap: 0, flexShrink: 0 }}>
        <span style={{ fontSize: small ? 11 : 12, color: dark ? "#8ecae6" : "#2a6f97", fontWeight: 600 }}>$</span>
        <input value={draft} onChange={(e) => setDraft(e.target.value)} onBlur={commit} onKeyDown={(e) => e.key === "Enter" && commit()}
          autoFocus placeholder="0" style={{ ...getInputStyle(dark), width: small ? 56 : 68, fontSize: small ? 11 : 12, padding: "3px 6px", textAlign: "right" }} />
      </div>
    );
  }
  return (
    <button onClick={() => { setDraft(value ? String(value) : ""); setEditing(true); }} title="Click to set budget"
      style={{ background: "none", border: `1px dashed ${dark ? "#555" : "#ccc"}`, borderRadius: 4, padding: small ? "2px 6px" : "3px 8px", fontSize: small ? 11 : 12, color: (value || 0) > 0 ? (dark ? "#8ecae6" : "#2a6f97") : (dark ? "#555" : "#bbb"), cursor: "pointer", fontWeight: 600, flexShrink: 0, whiteSpace: "nowrap" }}>
    ...
    <div style={getSubItemRowStyle(dark)}>
      {isEditing ? (
        ...
        <>
          ...
          <StatusSelect value={subItem.status || "want"} onChange={(s) => onStatusChange(subItem.id, s)} small />
...
function AddSubItemForm({ onAdd, dark }) {
  const [name, setName] = useState(""); const [priority, setPriority] = useState(""); const [error, setError] = useState("");
  const handleAdd = () => { const t = name.trim(), p = Number(priority); if (!t) return setError("Name can't be empty."); if (!Number.isInteger(p) || p < 1 || p > 10) return setError("Priority must be 1–10."); onAdd({ name: t, priority: p }); setName(""); setPriority(""); setError(""); };
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 6 }}>
      <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
        <input value={name} onChange={(e) => setName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleAdd()}
          placeholder="Add sub-item..." style={{ ...getInputStyle(dark), flex: 1, minWidth: 100, fontSize: 13, padding: "7px 10px" }} />
...
function WishlistItem({ item, onDelete, onEdit, onEditSave, editingId, onEditCancel, onAddSub, onDeleteSub, onEditSub, onEditSubSave, onStatusChange, onSubStatusChange, onBudgetChange, onSubBudgetChange, onColorChange, onRootBudgetClick, dark, selected, onToggleSelect, selectMode, onDragStart, onDragOver, onDrop, dragOverId }) {
  ...
  return (
    <div style={{ ...getItemContainer(dark), outline: selected ? "2px solid #4285f4" : "none", borderTop: isDragOver ? "3px solid #4285f4" : `1px solid ${dark ? "#333" : "#eee"}`, transition: "border-top 0.1s" }}
      ...
      <div style={getItemRow(dark)}>
        {isEditing ? (
          ...
          <>
            ...
            {/* Budget: if has subs, show cap (clickable to change), else editable */}
            {hasSubs ? (
              <button onClick={() => onRootBudgetClick(item.id)} title="Click to change budget cap"
                style={{ background: "none", border: `1px solid ${dark ? "#555" : "#ccc"}`, borderRadius: 4, padding: "3px 8px", fontSize: 12, fontWeight: 600, color: dark ? "#8ecae6" : "#2a6f97", cursor: "pointer", flexShrink: 0, whiteSpace: "nowrap" }}>
                {fmtMoney(subTotal)} / {fmtMoney(item.budget || 0)}
              </button>
            ) : (
              <BudgetInput value={item.budget || 0} onChange={(v) => onBudgetChange(item.id, v)} dark={dark} />
            )}
            {/* Status: derived if has subs, manual if not */}
            {hasSubs ? (
              <StatusBadge status={rootStatus} />
            ) : (
              <StatusSelect value={item.status || "want"} onChange={(s) => onStatusChange(item.id, s)} />
            )}
  ...
  return (
    <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", marginTop: 16 }}>
      ...
      <button onClick={onToggleReversed} title={`Currently: ${getSortLabel(sortMode, reversed)}`} disabled={sortMode === "manual"}
        style={{ ...getInputStyle(dark), display: "inline-flex", alignItems: "center", justifyContent: "center", width: 42, height: 42, padding: 0, cursor: sortMode === "manual" ? "default" : "pointer", background: reversed ? "#e8f0fe" : (dark ? "#333" : "#fafafa"), borderColor: reversed ? "#4285f4" : (dark ? "#555" : "#e0e0e0"), color: reversed ? "#4285f4" : "#999", fontSize: 18, fontWeight: 700, transition: "all 0.15s", flexShrink: 0, opacity: sortMode === "manual" ? 0.4 : 1 }}>⇅</button>
    </div>
  );
}
...
export default function WishlistApp() {
  ...
  const [name, setName] = useState(""); const [priority, setPriority] = useState("");
  const [error, setError] = useState(""); const [editingId, setEditingId] = useState(null);
  const [searchQuery, setSearchQuery] = useState("");
  const [sortMode, setSortMode] = useState("priority"); const [reversed, setReversed] = useState(false);
  ...
  const [budgetPopup, setBudgetPopup] = useState(null); // { itemId, isFirstSub? }
  const [budgetExceeded, setBudgetExceeded] = useState(null); // { remaining }
  ...
  const handleAdd = useCallback(() => {
    ...
    setItems((prev) => sortItems([...prev, { id: generateId(), name: t, priority: p, subItems: [], createdAt: Date.now(), status: "want", budget: 0, chartColor: randomColor() }], sortMode, reversed));
    setName(""); setPriority(""); setError("");
  }, [name, priority, sortMode, reversed, setItems]);
  ...
  const handleColorChange = useCallback((id, c) => { setItems((prev) => prev.map((i) => (i.id === id ? { ...i, chartColor: c } : i))); }, [setItems]);

  // ── Sub-item budget with cap enforcement ──
  const handleSubBudgetChange = useCallback((pid, sid, newVal) => {
    ...
    if (cap > 0 && otherTotal + newVal > cap) {
      setBudgetExceeded({ remaining: Math.max(0, cap - otherTotal) });
      return;
    }
    setItems((prev) => prev.map((i) => { if (i.id !== pid) return i; return { ...i, subItems: i.subItems.map((x) => (x.id === sid ? { ...x, budget: newVal } : x)) }; }));
  }, [items, setItems]);

  // ── Root budget click (change cap) ──
  const handleRootBudgetClick = useCallback((id) => {
    if (dontAskBudgetChange) {
      // Skip popup, go straight to edit
      setBudgetPopup({ itemId: id, isFirstSub: false, skipMessage: true });
    } else {
      setBudgetPopup({ itemId: id, isFirstSub: false });
    }
  }, [dontAskBudgetChange]);

  // ── Save budget cap from popup ──
  ...
  // ── Add sub-item (with budget cap popup on first sub) ──
  const handleAddSub = useCallback((pid, sd) => {
    ...
    setItems((prev) => sortItems(prev.map((i) => {
      ...
      return { ...i, subItems: sortSubItems([...i.subItems, { id: generateId(), ...sd, createdAt: Date.now(), status: "want", budget: 0 }], sortMode, reversed) };
    }), sortMode, reversed));

    // Show budget cap popup on first sub-item added (if not suppressed)
    if (isFirst && !dontAskBudgetCap) {
      setBudgetPopup({ itemId: pid, isFirstSub: true });
    }
  ...
  const handleDrop = (e, targetId) => {
    ...
    const order = displayedItems.map((i) => i.id); const fi = order.indexOf(dragItem.current); const ti = order.indexOf(targetId);
  ...
  const totalItems = items.length; const shownItems = displayedItems.length; const isSearching = searchQuery.trim().length > 0;

  // Get item name for budget popup
  const budgetPopupItem = budgetPopup ? items.find((i) => i.id === budgetPopup.itemId) : null;

  return (
    <div style={{ background: dark ? "#1a1a2a" : "#f0f0f0", minHeight: "100vh", transition: "background 0.2s" }}>
      <ShineStyle />
      <div style={getContainer(dark)}>
        ...
        {budgetPopup && budgetPopupItem && (
          <BudgetCapPopup
            itemName={budgetPopupItem.name}
            currentBudget={budgetPopupItem.budget}
            onSave={handleBudgetCapSave}
            onCancel={() => setBudgetPopup(null)}
            dark={dark}
            showDontAsk={budgetPopup.isFirstSub}
            onDontAsk={() => setDontAskBudgetCap(true)}
          />
        )}
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
          ...
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            ...
            <button onClick={() => setDark((d) => !d)} title={dark ? "Light mode" : "Dark mode"} style={{ ...getBtnIcon(dark), fontSize: 18 }}>{dark ? "☀️" : "🌙"}</button>
          </div>
        </div>
        <p style={{ margin: "0 0 20px", fontSize: 13, color: dark ? "#777" : "#888" }}>Add items and rank them 1–10. Click ▶ to expand sub-items.</p>
        ...
        {totalItems > 0 && (
          <>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 6 }}>
              <p style={{ fontSize: 11, color: dark ? "#666" : "#bbb", margin: 0 }}>{getSortLabel(sortMode, reversed)}</p>
              {!selectMode ? (
                ...
                <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                  ...
                  <button onClick={requestBulkDelete} disabled={selectedIds.size === 0}
                    style={{ fontSize: 12, fontWeight: 600, padding: "4px 10px", borderRadius: 6, border: "none", background: selectedIds.size > 0 ? "#c0392b" : (dark ? "#444" : "#ddd"), color: selectedIds.size > 0 ? "#fff" : "#999", cursor: selectedIds.size > 0 ? "pointer" : "default" }}>Delete ({selectedIds.size})</button>
        ...
        {isSearching && totalItems > 0 && (
          <p style={{ fontSize: 12, color: dark ? "#777" : "#999", margin: "4px 0 0" }}>
...
// ─── Styles ───
const getContainer = (dark) => ({ maxWidth: 1100, margin: "0 auto", padding: "40px 48px", fontFamily: "'Inter', system-ui, -apple-system, sans-serif", background: dark ? "#1e1e2e" : "#fff", minHeight: "100vh", boxSizing: "border-box", transition: "background 0.2s, color 0.2s" });
const getInputStyle = (dark) => ({ padding: "10px 12px", fontSize: 14, border: `1.5px solid ${dark ? "#444" : "#e0e0e0"}`, borderRadius: 8, outline: "none", transition: "border 0.15s", background: dark ? "#2a2a3e" : "#fafafa", color: dark ? "#e0e0e0" : "#333" });
...
const getBtnCancel = (dark) => ({ padding: "6px 14px", fontSize: 13, fontWeight: 600, background: dark ? "#444" : "#eee", color: dark ? "#ccc" : "#555", border: "none", borderRadius: 6, cursor: "pointer" });
const getItemContainer = (dark) => ({ borderRadius: 10, border: `1px solid ${dark ? "#333" : "#eee"}`, overflow: "hidden" });
const getItemRow = (dark) => ({ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", background: dark ? "#262638" : "#f8f9fa" });
const getSubPanel = (dark) => ({ padding: "8px 14px 12px 42px", background: dark ? "#1e1e2e" : "#f1f3f5", borderTop: `1px solid ${dark ? "#333" : "#e9ecef"}`, display: "flex", flexDirection: "column", gap: 6 });
const getSubItemRowStyle = (dark) => ({ display: "flex", alignItems: "center", gap: 8, padding: "6px 10px", background: dark ? "#2a2a3e" : "#fff", borderRadius: 6, border: `1px solid ${dark ? "#333" : "#e9ecef"}` });
